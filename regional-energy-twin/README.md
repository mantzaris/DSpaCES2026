# Regional energy twin research

This paper is self-contained under `regional-energy-twin/`. From the shared
DSpaCES2026 repository root, run `cd regional-energy-twin` before the project
commands below. Paths in earlier reports, ledgers and artifact manifests remain
relative to this project directory. The shared `.git` directory stays one level
above so other papers can use sibling directories.

The reorganization preserves all earlier research candidates and results within
this project; it does not authorize another study or move the remote pod.
