...
def main():
    ...
    folder = root/'results/acquisition'
    folder.mkdir(parents=True, exist_ok=True)
    path = folder/'resource_ledger.json'
    prior = json.loads((root/'results/shock/resource_handoff.json').read_text())
    with (folder/'.ledger.lock').open('w') as lock:
        ...
        if args.close:
            ...
            timeout = min(args.timeout, remaining - (0 if args.label.startswith("final") else 300))
            if timeout <= 0:
                raise SystemExit("Five-minute handoff reserve reached")
